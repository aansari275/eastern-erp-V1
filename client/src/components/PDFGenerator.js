...
const PDFGenerator = ({ rug, isOpen, onClose }) => {
    const generatePDF = () => {
        ...
        if (rug.designName)
            productDetails.push(['Design Name:', rug.designName]);
        if (rug.carpetNo)
            productDetails.push(['Carpet No.:', rug.carpetNo]);
        if (rug.construction)
            productDetails.push(['Construction:', rug.construction]);
        if (rug.quality)
            productDetails.push(['Quality:', rug.quality]);
        if (rug.color)
            productDetails.push(['Color:', rug.color]);
        if (rug.shadeCardNo)
            productDetails.push(['Shade Card No.:', rug.shadeCardNo]);
        if (rug.unfinishedGSM > 0)
            productDetails.push(['Unfinished GSM:', Math.round(rug.unfinishedGSM).toString()]);
        if (rug.finishedGSM > 0)
            productDetails.push(['Finished GSM:', Math.round(rug.finishedGSM).toString()]);
        if (rug.washed !== undefined)
            productDetails.push(['Washed Y/N:', rug.washed ? 'Y' : 'N']);
        pdf.setFontSize(9);
        pdf.setFont('helvetica', 'normal');
        ...
        if (materials.length > 0) {
            ...
            pdf.setFontSize(9);
            pdf.setFont('helvetica', 'normal');
            warpMaterials.forEach((material, index) => {
            ...
            weftMaterials.forEach((material, index) => {
        ...
        if (rug.pileGSM > 0)
            constructionData.push(['Pile GSM:', rug.pileGSM.toString()]);
        if (rug.warpIn6Inches)
            constructionData.push(['Warp in 6 inches:', rug.warpIn6Inches]);
        if (rug.weftIn6Inches)
            constructionData.push(['Weft in 6 inches:', rug.weftIn6Inches]);
        if (rug.pileHeightMM > 0)
            constructionData.push(['Pile Height in MM:', `${rug.pileHeightMM} mm`]);
        if (rug.totalThicknessMM > 0)
            constructionData.push(['Total thickness in MM:', `${rug.totalThicknessMM} mm`]);
        ...
        // EDGE DETAILS
        const edgeData = [];
        if (rug.edgeLongerSide)
            edgeData.push(['Longer side:', rug.edgeLongerSide]);
        if (rug.edgeShortSide)
            edgeData.push(['Short side:', rug.edgeShortSide]);
        if (rug.fringesHemLength)
            edgeData.push(['Fringes/Hem length:', rug.fringesHemLength]);
        if (rug.fringesHemMaterial)
            edgeData.push(['Fringes/Hem Material:', rug.fringesHemMaterial]);
        ...
        if (processFlow.length > 0) {
            ...
            pdf.setFontSize(9);
            pdf.setFont('helvetica', 'normal');
            processFlow.forEach((process, index) => {
    ...
    if (!isOpen)
        return null;
    return (_jsx("div", { className: "fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50", children: _jsxs("div", { className: "bg-white rounded-lg p-6 max-w-md w-full mx-4", children: [_jsxs("h3", { className: "text-lg font-semibold mb-6", children: ["Download ", rug.designName || 'Rug', " TED"] }), _jsxs("div", { className: "flex gap-3", children: [_jsx("button", { onClick: generatePDF, className: "flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700", children: "Download PDF" }), _jsx("button", { onClick: onClose, className: "flex-1 bg-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-400", children: "Cancel" })] })] }) }));
};

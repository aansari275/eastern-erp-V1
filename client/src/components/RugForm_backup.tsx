...
const RugForm = ({ rug, onSave, onReset, onBack }) => {
    ...
    const compressImage = (file, maxSizeKB = 2048) => {
        return new Promise((resolve, reject) => {
            ...
            img.onload = () => {
                ...
                const sizeKB = (compressedDataUrl.length * 3) / 4 / 1024; // Rough base64 to KB conversion
                if (sizeKB > maxSizeKB) {
    ...
    const handleImageUpload = async (imageType, file) => {
        try {
            ...
            // Always try compression first for better storage efficiency
            try {
                const compressedBase64 = await compressImage(file, 2048);
                console.log(`Compressed ${imageType} to ${Math.round((compressedBase64.length * 3) / 4 / 1024)}KB`);
                setFormData(prev => {
                    const newImages = { ...prev.images, [imageType]: compressedBase64 };
                    console.log(`Updated formData.images for ${imageType}:`, !!newImages[imageType]);
                    return {
                        ...prev,
                        images: newImages
                    };

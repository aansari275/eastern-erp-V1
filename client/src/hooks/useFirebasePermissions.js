...
import { DEPARTMENT_TABS } from '@shared/permissions';
// Convert Firebase user to UserPermission format
const convertFirebaseUserToPermission = (firebaseUser) => {
    // Parse the role - default to 'Viewer' if not recognized
    const role = firebaseUser.role === 'Editor' ? 'Editor' : 'Viewer';
    ...
    if (firebaseUser.departments && Array.isArray(firebaseUser.departments)) {
        departments = firebaseUser.departments.filter((dept) => ['Products', 'Production', 'Quality', 'Compliance'].includes(dept));
    }
...
export const useFirebasePermissions = (userId) => {
    ...
    const users = firebaseUsers.map(convertFirebaseUserToPermission);
    ...
    const updateUserDepartmentsMutation = useMutation({
        mutationFn: async ({ userId, departments }) => {
            const response = await fetch(`/api/users/${userId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (!response.ok)
                throw new Error('Failed to update user departments');
            return response.json();
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['/api/users'] });
    ...
    const canEditInDepartment = (department) => {
        if (!userPermission || !userPermission.isActive)
            return false;
    ...
    const canAccessTab = (tabId) => {
        ...
        for (const [department, tabs] of Object.entries(DEPARTMENT_TABS)) {
            ...
            if (tab) {
                ...
                if (!userPermission.departments.includes(department))
                    return false;
    ...
    return {
        ...
        updateUserDepartments: (userId, departments) => updateUserDepartmentsMutation.mutate({ userId, departments }),
        toggleUserActive: (userId, isActive) => toggleUserActiveMutation.mutate({ userId, isActive }),
    };
};

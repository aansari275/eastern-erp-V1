...
import { onAuthStateChanged, signInWithRedirect, signInWithPopup, getRedirectResult, GoogleAuthProvider, type User } from 'firebase/auth';
...
export const useAuth = () => {
  ...
  const login = async () => {
    try {
      ...
      // Handle unauthorized domain error specifically - don't show alert, let component handle it
      if (error.code === 'auth/unauthorized-domain') {
        throw error; // Re-throw for component to handle with modal
      }
      ...
      if (error.code === 'auth/popup-blocked' || 
        ...
        try {
          ...
          if (redirectError.code === 'auth/unauthorized-domain') {
            throw redirectError; // Re-throw for component to handle with modal
          } else {
            throw redirectError; // Re-throw for component to handle
          }
        }
      } else {

...
export const useAutoSave = ({
  ...
  const saveData = useCallback(async (saveData: any) => {
    if (!enabled || !user || isLoadingRef.current) return;
 ...
 */
export const useAutoSaveId = (prefix: string, userId?: string) => {
  const { user } = useAuth();
  const effectiveUserId = userId || user?.uid || 'anonymous';
  
  return useCallback((additionalId?: string) => {

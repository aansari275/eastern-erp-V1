...
export function canViewTab(user, tabName) {
    return user?.tabs?.includes(tabName) ?? false;
}
export function canView(user, permission) {
    if (!user)
        return false;
...
export function canEdit(user, permission) {
    if (!user)
        return false;

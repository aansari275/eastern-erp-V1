...
import { masterDefects, type InsertMasterDefect } from './shared/schema';

// Master defects data extracted from the provided image
const masterDefectsData: InsertMasterDefect[] = [
...
async function setupMasterDefects() {
  try {
    console.log('🚀 Setting up master defects database...');
    
    // Clear existing defects (if any)
    console.log('🗑️ Clearing existing master defects...');
    await db.delete(masterDefects);
    ...
    for (const defect of masterDefectsData) {
      await db.insert(masterDefects).values(defect);
...
setupMasterDefects()
  .then(() => {
    console.log('🎉 Master defects database setup completed!');
    process.exit(0);
  })
  .catch((error) => {

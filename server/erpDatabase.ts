...
export async function getMaterials() {
  try {
    ...
    const query = `
      SELECT DISTINCT 
        sitem_nm as materialName,
        it_rate as rate,
        s_nm as supplierName
      FROM vopspurchase 
      WHERE sitem_nm IS NOT NULL 
...
export async function getMaterialRate(materialName: string) {
  try {
    const connection = await initializeERPDatabase();
    const request = connection.request();
...
export async function getOPSOrders(startDate?: string, endDate?: string) {
  try {
    const connection = await initializeERPDatabase();
    const request = connection.request();
...
export async function getOPSOrderDetails(boId: string) {
  try {
    const connection = await initializeERPDatabase();
    const request = connection.request();
...
// Get buyers from ERP database
export async function getBuyersFromERP() {
  try {
    const connection = await initializeERPDatabase();
    const request = connection.request();
    ...
    const result = await request.query(query);
    console.log(`Found ${result.recordset.length} buyers from ERP`);
    
    return result.recordset.map((record: any) => ({
      code: record.buyerCode?.trim() || '',
      name: record.buyerName?.trim() || '',
    }));
  } catch (error) {
    console.error('Error fetching buyers from ERP:', error);
    throw error;
  }
}

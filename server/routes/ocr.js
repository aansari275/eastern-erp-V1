...
router.get("/verify-storage", (req, res) => {
    ...
    bucket.getFiles({ prefix: "uploads/" }, (err, files) => {
        ...
        console.log("Files in uploads/:", files.map((f) => f.name));
        bucket.getFiles({ prefix: "images/" }, (err2, imageFiles) => {
            if (err2) {
                console.error("Images verification error:", err2);
            }
            console.log("Files in images/:", imageFiles.map((f) => f.name));
            res.send(`Found ${files.length} files in uploads/ and ${imageFiles.length} in images/`);
        });
    });
});

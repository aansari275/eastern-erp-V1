...
router.post('/escalate', async (req, res) => {
  try {
    const {
      ...
      defects = [],
      escalationContacts = [],
      requestingUser = 'lab.technician@easternmills.com'
    } = req.body;
    ...
    const defectsList = defects.map((defect: any, index: number) => 
      `${index + 1}. ${defect.name} (${defect.severity})`
    ).join('\n');
    ...
    const companyKey = company as 'EHI' | 'EMPL';
    ...
    const emailPromises = allContacts.map(async (email: string) => {
  ...
  try {
    ...
    if (decision === 'rejected' && resend && process.env.RESEND_API_KEY) {
      ...
      if (level2Recipients.length > 0) {
        ...
        const emailPromises = level2Recipients.map(email => 
          resend.emails.send({
            to: email.trim(),
            from: 'quality@easternmills.com',
...
router.post('/operations-decision', async (req, res) => {
  try {
    const {
      recordId,
      labReportNumber,
      decision, // 'approved' or 'final_rejected'

...
import type { SampleDispatchInspection } from '@shared/schema';
...
router.get('/', async (req, res) => {
  try {
    ...
    const inspections: SampleDispatchInspection[] = [];
    snapshot.forEach((doc: any) => {
      inspections.push({ id: doc.id, ...doc.data() } as SampleDispatchInspection);
    });
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const inspection = { id: doc.id, ...doc.data() } as SampleDispatchInspection;
...
router.post('/', async (req, res) => {
  try {
    ...
    const createdInspection = { id: createdDoc.id, ...createdDoc.data() } as SampleDispatchInspection;
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const updatedInspection = { id: updatedDoc.id, ...updatedDoc.data() } as SampleDispatchInspection;
...
router.get('/:id/pdf', async (req, res) => {
  try {
    ...
    if (inspection.weight) details.push(['Weight (kg):', inspection.weight]);
    if (inspection.gsm) details.push(['GSM:', inspection.gsm]);
    if (inspection.pile) details.push(['Pile Height (mm):', inspection.pile]);
    if (inspection.gauge) details.push(['Gauge:', inspection.gauge]);
    ...
    if (inspection.defects && inspection.defects.length > 0) {
      ...
      inspection.defects.forEach((defect, index) => {
        const severityColor = 
          defect.severity === 'Critical' ? rgb(0.8, 0.2, 0.2) :

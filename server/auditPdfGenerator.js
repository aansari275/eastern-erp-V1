...
const registerHandlebarsHelpers = () => {
    ...
    Handlebars.registerHelper('getScoreLabel', (score) => {
        if (score >= 90)
            return 'Outstanding';
        if (score >= 70)
            return 'Satisfactory';
    ...
    Handlebars.registerHelper('getOverallResult', (score) => {
...
const registerSectionHelpers = () => {
    Handlebars.registerHelper('getSectionScore', (items) => {
        const total = items.length;
        const passed = items.filter(item => item.response === 'Yes').length;
        return { total, passed };
    });
    Handlebars.registerHelper('getSectionWeight', (index, totalSections) => {
        // Distribute weights evenly
        return Math.round(100 / totalSections);
    });
    Handlebars.registerHelper('getSectionWeightedScore', (items, index, totalSections) => {
...
export class AuditPDFGenerator {
     ...
     */
    convertComplianceAuditToAuditData(rawData, auditId) {
        ...
        if (rawData.parts && Array.isArray(rawData.parts)) {
            checklist = rawData.parts.map((part, index) => ({
     ...
     */
    async generateAuditPDF(auditId) {
        try {
     ...
     */
    async generatePDFUsingFallback(data) {
        try {
            ...
            yPos += 20;
            // Add sections
            fallbackData.checklist.forEach((section, index) => {
                if (yPos > 250) {
                    doc.addPage();
                    yPos = 20;
                }
     ...
     */
    async getPreviousAudits(company, limit = 10) {
        try {
...
export const generateAuditPDF = (auditId) => auditPDFGenerator.generateAuditPDF(auditId);

// CSV-based defects data for process-specific quality inspections
...
function getSeverity(defectName: string): 'Critical' | 'Major' | 'Minor' {
  ...
  if (critical.some(c => defectName.includes(c))) return 'Critical';
  if (minor.some(m => defectName.includes(m))) return 'Minor';
  return 'Major';
}

// Master defects from CSV data
export const CSV_DEFECTS: CSVDefect[] = [
...
export function getCSVDefectsByProcess(process: string): Array<{defectCode: string, defectName: string, severity: 'Critical' | 'Major' | 'Minor'}> {
  return CSV_DEFECTS
...
export function getDefectSeverityColor(severity: string): string {
  switch (severity) {
    case 'Critical':
